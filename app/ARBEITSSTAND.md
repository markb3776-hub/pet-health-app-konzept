...
## ABSCHLUSS Teilauftrag 4.3 — Notfallpass + QR-Code (09.07.2026)
Umgesetzt gemäß `notfallpass_design_spezifikation.md` und Screen-Flow 2.6:
- **Zentrales Pass-Datenmodul** src/emergency/passData.ts: EINE Datenquelle (loadPassData) für Bildschirm, QR-Payload (buildQrPayload) und PDF (buildPassHtml) — Abweichungen zwischen den drei Darstellungen technisch ausgeschlossen. HTML-Escaping (XSS-sicher), Doktrin-Fußnote (ersetzt keinen EU-Heimtierausweis). SPECIALIST_SPECIES: vogel/kaninchen/reptil/meerschweinchen/chinchilla/degu/frettchen.
- **EmergencyPassScreen komplett neu:** Passkarte (Foto Tap→Vollbild, ehrlicher Initial-Platzhalter statt Fremdbild), Signalement (Geboren/Geschlecht/Kastration/Rasse/Fellfarbe), Erkennungsmerkmale IMMER sichtbar, Chip-Kopieren (expo-clipboard), Notfall-Block feste Reihenfolge (Allergien→Dauermedikation→Vorerkrankungen→Impfstatus→letzte Werte→Spezialist bei Spezialisten-Arten→Kontakt), QR-Modal (react-native-qrcode-svg, offline, deutscher Klartext ~320 Zeichen, keine Server-URL, Browser-Freigabe ehrlich als „nach dem Prototyp"), PDF-Teilen (expo-print + expo-sharing, A4, Graustufen-tauglich, Foto base64 via expo-file-system File.base64() — API in Paket-Typen verifiziert), Mehrtier-Umschalt-Chips, Querformat 2-spaltig.
- **Neue Felder (additive DB-Migration 003):** pets.coat_color, pets.vet_practice_name, pets.vet_practice_phone; EditPetScreen erweitert (Fellfarbe nur Nicht-Habitat, Stamm-Tierarztpraxis). Halter-Telefon: profileStore (Key simplypet.owner_phone) + MoreScreen (Name+Telefon gemeinsam bearbeitbar, Abbrechen setzt beide zurück, ehrlicher Hinweis wenn Telefon fehlt).
- **Neue Pakete:** expo-print, expo-sharing, expo-clipboard.
- **Prüfung bestanden:** tsc 0 Fehler, expo-doctor 20/20, expo export android (.hbc), 24/24 QR/PDF-Logiktests (voll/leer/XSS/Spezialist/Signalement), SQL-Spalten-Abgleich, Spezifikations-Greps (eine Zeitquelle, Sektions-Reihenfolge, Zwei-Tap auf allen Haupt-Tabs). Protokoll: `pruefprotokoll_teilauftrag_4_3.md`. Gefundene+korrigierte Punkte: Abbrechen-Knopf setzte Telefon nicht zurück (behoben).
Nächster Schritt: 4.4 interne Prüfung + APK (inkl. Tageswechsel-Testfall und Mehrarten-Stabilitätstest gemäß Roadmap Schritt 4 Punkte 5+6).

## ABSCHLUSS Teilauftrag 4.4 — Interne Gesamtprüfung + APK (10.07.2026)
Durchgeführt gemäß Roadmap Schritt 4 Punkte 5+6, Strukturkonzept Abschn. 7, pruefdoktrin_eingabe_stabilitaet.md:
- **11 Prüfblöcke, alle bestanden** (Logiktests je 3 Durchgänge): tsc 0 Fehler; Draft-Wiring aller 9 Formulare; 14-Tierarten-Register (17/17); Tageswechsel-Logik (AppState-Listener + 60s-Intervall, 6/6); Mehrarten-SQL-Szenario Hund/Hamster/Aquarium inkl. Archiv-Störprobe + 50x-Wechsel (11/11); Nachtragen-Testfall; Leer-/Maximalzustand-Scroll; Offline-Start (kein Netzaufruf im App-Code); große Schrift (Skalierung nirgends deaktiviert, minTouchTarget in 18 Dateien); expo export + expo-doctor 20/20.
- **Fund 1 KORRIGIERT — Farbpalette:** violett/braun in Graustufen identisch (0.004), blau/violett bei Deuteranopie zu nah (13.7). Sechs Töne rechnerisch neu (blau #2F6495, gruen #365233, violett #61517A, petrol #225A5A, braun #A7795E, nachtblau #242D4E). Final 4/4: Graustufen ≥0.0168, Deuteranopie ≥28.8, Weiß-Kontrast ≥3.12, Signalrot-Abstand 99.
- **Fund 2 KORRIGIERT — Zwei-Tap:** Tierakte hatte keinen direkten Notfall-Zugang (Strukturkonzept: Knopf auf JEDEM Bildschirm). EmergencyFab um petId-Prop erweitert + in PetFileScreen eingebaut → öffnet direkt den Pass DIESES Tieres (1 Tap).
- **APK gebaut + verifiziert:** app-release.apk, 134 MB, de.simplypet.app 0.1.0, minSdk 29 (Android 10+), targetSdk 35, apksigner verify PASS. Build-Weg: expo prebuild + Gradle lokal (JDK 17; SDK-Plattform 36 wegen androidx.camera-Anforderung — compileSdk 36, targetSdk bleibt 35). Debug-Signatur (für Testgeräte; Play-Signatur erst bei Veröffentlichung). APK bewusst NICHT im Repo (Größe) — direkt an Projektinhaber übergeben; reproduzierbar via `cd app && npm install && npx expo prebuild -p android && cd android && ./gradlew assembleRelease` (compileSdk in gradle.properties auf 36 setzen).
- **package.json-Hinweis:** prebuild stellte scripts android/ios auf `expo run:android`/`expo run:ios` um (Expo-Standard nach Prebuild) — übernommen.
- **Ehrlich delegiert an Nutzertest** (pruefprotokoll_prototyp_nutzertest.md): Berechtigungsdialoge, echtes Über-Nacht-Verhalten (Block 4d), Systemschrift-Maximum am Gerät, QR-Scan mit Zweitgerät, PDF-Druckbild, App-Kill-Störfälle.
Protokoll: `pruefprotokoll_teilauftrag_4_4.md`. **Schritt 4 damit komplett. Nächster Schritt: Schritt 5 — Nutzertest auf beiden Android-Geräten.**
