...
| E-97 | Dokumentenscan mit OCR-Erkennung | Neues Feature: Nutzer fotografiert Impfpass-Seite → App erkennt per On-Device-OCR (Google ML Kit Text Recognition, offline) die Daten und füllt Impfeinträge automatisch vor. Ablauf: (1) Foto machen/wählen, (2) OCR liest Text, (3) Heuristik/Regex ordnet Felder zu (Impfstoff-Namen aus bekannter Liste, Datum-Muster, Chargen-Format), (4) Vorschau "Stimmt das?" mit Korrekturmöglichkeit, (5) Bestätigung → Eintrag wird angelegt. Tier-Zuordnung wird vorher abgefragt. Kein Server nötig (Doktrin: keine INTERNET-Permission). Handschrift-Erkennung als optionale Erweiterung (Cloud-basiert, nur auf Nutzer-Wunsch). | Sprachmemo 20.07.2026: "Dokumentenscan muss noch eingefügt werden. Wenn das Dokument eingescannt ist, ist zu fragen, welchem Tier das Dokument zugeordnet werden soll." + Impfpass-Analyse: Aufkleber (Nobivac, Purevax) sind gut OCR-lesbar. Handschrift (Daten) ist schwieriger aber mit Vorschau-Bestätigung lösbar. |
